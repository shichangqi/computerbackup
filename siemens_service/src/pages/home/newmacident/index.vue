...
<script>
  ...
  export default {
    components: {
      ...
      data(){
        return{
          ...
          productmodalarray:[],
          productmodal:'',
          faulttypearray:['外观损伤或生锈/外包装不完整','机器有灰尘','附件缺失 ','其他'],
          faultType:'',
          num:0,
          date:'',
          timearray: [
          ...
          pickertype:1,
          productpicker:[],
          instype:['洗衣机','洗干一体机','干衣机','上下两门冰箱','上下三门冰箱','对开门冰箱','多门冰箱','吸油烟机','灶具','消毒柜','洗碗机','烤箱','蒸箱','微波炉','电热水器','酒柜','暖碟抽屉'],
    ...
    async onLoad(option){
      ...
      let tiptime = wx.getStorageSync('tiptime');
      if(now == tiptime){
        this.isclose = false;
      }
    ...
    methods:{
      ...
      async getPhoneNumber(){
        ...
        let phone = wx.getStorageSync('phone')
                if(_this.agree1 && _this.agree2){
                  ...
                  let desc = `问题描述: 微信报单,新机鉴定, 备注:${_this.desc}`+'期望上门时间:'+_this.date+'  '+_this.timetext+'优先联系电话:'+_this.address.mobile+'微信授权手机号：'+ phone;
                  ...
                  saveOrder(datas).then(res=>{
                    if (res.data.flag == 1100) {
                      ...
                      wx.removeStorageSync("address");
                      wx.removeStorageSync("selectPro");
                      reLaunch(`/pages/newmacsuccess/main?tip=${JSON.stringify(_this.tip)}&address=${JSON.stringify(_this.address)}`)
                      })
      ...
      close(){
        ...
        wx.setStorageSync('tiptime', date)
      },
      preview(path){
        let list = [path];
        wx.previewImage({
      ...
      // submit(){
      //   navigate('/pages/newmacsuccess/main')
      // },
      toproductrepair(){
      ...
      changePro(){
        navigate(`/pages/selectProduct/main?id=${this.pro.id}&protype=${JSON.stringify(this.instype)}&type=3`)
      },
      faulttypechange(e){
