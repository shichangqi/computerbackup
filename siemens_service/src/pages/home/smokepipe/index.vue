...
export default {
  ...
  async onLoad(option) {
    ...
    let tiptime = wx.getStorageSync('smoketime');
    if(now == tiptime){
      this.isclose = false;
    }
    ...
    await getInstallProductTypes(data).then(res => {
      ...
      for (let i = 0; i < res.data.data.length; i++) {
        if (this.productTypes[i].typeName == "吸油烟机") {
          this.pdv = this.productTypes[i].pdv;
          this.scanImg = this.productTypes[i].scanImg;
          this.productId = this.productTypes[i].typeId;
          this.productType = "吸油烟机";
  ...
  methods: {
    ...
    close(){
      ...
      wx.setStorageSync('smoketime', date)
    },
    ...
    bind (id, fd, sn, type) {
      bindProduct({
        ...
        if (res.data.code == -1) {
          ...
          if(res.data.data.productName != '吸油烟机'){
      ...
      let _this = this;
       ...
       let phone = wx.getStorageSync('phone')
              if (_this.agree1 && _this.agree2) {
                ...
                let desc = `问题描述: 微信报单,预埋烟管,${_this.arrival}到货, 备注:${_this.otherRequest}`+'期望上门时间:'+_this.date+'  '+_this.timetext+'优先联系电话:'+_this.address.mobile+'微信授权手机号：'+ phone;
                ...
                saveOrder(data).then(res => {
                  if (res.data.flag == 1100) {
                    ...
                    wx.removeStorageSync("address");
                    wx.removeStorageSync("selectPro");
                    reLaunch(`/pages/smokepipesuccess/main?tip=${JSON.stringify(_this.tip)}&address=${JSON.stringify(_this.address)}`)
                    })
    ...
    selectcontactor() {
      wx.navigateTo({ url: "/pages/contactWay/main" });
    },
    changePro() {
      navigate("/pages/selectProduct/main?type=4");
    },
    timechange(e) {
