<template>
    <div class="repairnothousehold">
      <div class="notqr">
        ...
        <div class="tips"v-if="isclose">
          <div class="content">
            ...
            <p  class="down" @tap="godown">
              <img src="../../../static/images/down_icon.png" v-if="toup">
              <img src="../../../static/images/up_icon.png" v-else>
            </p>
          </div>
        </div>
        <div class="calendar1" v-if="calendar1"catchtouchmove="ture">
          <div class="container1" @tap.stop="calendar1 = false"></div>
          <div class="container">
        ...
        <div class="calendar2" v-if="calendar2"catchtouchmove="ture">
          ...
          <div class="container">
            <Calendar @select="selectdate2" :begin="begin"ref="calendar2"/>
          </div>
        </div>
      ...
      <div class="contact">
        <div class=" contactpanel" @tap="selectcontactor">
          <p><span class="panel_name">拆机地址</span>
      ...
      <div class="form2">
        <div class="panel"@tap="expectdateChange">
          <!--<picker mode="date" :value="date"  @change="dateChange":start="startdate">-->
          <p class="panelmar"><span class="panel_name">拆机时间</span>
              <span class="check_name">{{date?date:'请选择拆机时间'}} <img src="../../../static/images/right_icon.png"></span>
          </p>
          <!--</picker>-->
        </div>
        <div class="panel">
          <!--<picker  :value="index" :range="timearray" @change="timechange">-->
          <p @tap="timeclick"><span class="panel_name">期望时段</span>
      ...
      <div class="contactmove">
        <div class=" contactpanel" @tap="selectcontactor2">
          <p><span class="panel_name">二次安装地址</span>
            <span class="check_name" >{{address2.name?'已选择':'请选择'}}<img src="../../../static/images/right_icon.png"></span>
          </p>
          <div class="contact"v-if="address2.name" >
            <div class="info">
              <p><span>{{address2.surname+address2.name}}{{address2.sex == 1?'先生':'女士'}}</span><span>{{address2.mobile}}</span></p>
              <p class="address">{{address2.province}}{{address2.city}}{{address2.country}}{{address2.addr}}</p>
            </div>
          </div>
        </div>
      </div>
      <div class="moveform2">
        <div class="panel"@tap="dateChange">
          <!--<picker mode="date" :value="movedate"  @change="movedateChange":start="startdate">-->
            <p class="panelmar"><span class="panel_name">二次安装时间</span>
              <span class="check_name">{{movedate?movedate:'请选择拆机时间'}} <img src="../../../static/images/right_icon.png"></span>
            </p>
          <!--</picker>-->
        </div>
        <div class="panel">
          <!--<picker  :value="index" :range="movetimearray" @change="movetimechange">-->
            <p @tap="timeclick2"><span class="panel_name">期望时段</span>
              <span class="check_name">{{movetimetext?movetimetext:'请选择服务时段'}} <img src="../../../static/images/right_icon.png"></span>
            </p>
          <!--</picker>-->
        </div>
      </div>
  ...
  <div class="otherrequire"v-if="!show && calendar1 == false && calendar2 == false && spicker == false">
    ...
    <textarea placeholder="选填"  placeholder-style="font-size:14px;color:#B8C3CA;" v-model="desc"></textarea>
      ...
      <div class="contact">
        <div class="panel contactpanel" @tap="servicestandar">
          <p><span class="panel_name">客户服务收费标准</span>
            <span class="check_name" >请查看 <img src="../../../static/images/right_icon.png"></span>
          </p>
        </div>
      </div>
      <div class="bottom">
...
<script>
  import {navigate,reLaunch,formatTime,formatNumber,arrayTime} from "../../../utils/index";
  import {getInstallProductTypes,getProductModal,getToken,saveOrder,getUserPhone,bindProduct,getservicecontent,wxordersave} from "../../../api/api";
  ...
  export default {
    components: {
      ...
      data(){
        return{
          ...
          productmodalarray:[],
          productmodal:'',
          faulttypearray:['机器有灰尘','附件缺失','外观损伤或生锈','其他'],
          faultType:'',
          num:0,
          date:'',
          timearray:[
    ...
    async onLoad(option){
      ...
      let tiptime = wx.getStorageSync('movetime');
      if(now == tiptime){
        this.isclose = false;
      }
      ...
      await getInstallProductTypes(data).then(res=>{
        ...
        for (let i=0;i<res.data.data.length;i++) {
          if(res.data.data[i].typeName != '家用小电器' && res.data.data[i].typeName != '上下两门冰箱' && res.data.data[i].typeName != '上下三门冰箱'){
        ...
        this.productpicker = this.producttypearray;
          // this.producttypearray.push(res.data.data[i].typeName)
    ...
    onShow(){
      ...
      let address2 = wx.getStorageSync('address2');
      if(address2){
        this.address2 = address2;
        // this.movetimetext = '';
        if(address2.province == '新疆'){
          let xjtimes=   [
            {
              label:'11:00-13:00',
              value:0
    ...
    methods:{
      ...
      timeclick2(){
        this.productpicker = this.movetimearray;
        this.$refs.typePicker.show();
        this.pickertype = 3
        this.spicker = true
      },
      ...
      selectdate(v1){
        ...
        this.date= '';
        // this.begin2 = arrayTime(new Date(Date.parse(this.date)+86400000))
      ...
      selectdate2(v1){
        this.movedate = formatNumber(v1[0])+'/'+formatNumber(v1[1])+'/'+formatNumber(v1[2])
      ...
      close(){
        ...
        wx.setStorageSync('movetime', date)
      },
      preview(path){
        let list = [path];
        wx.previewImage({
      ...
      async getPhoneNumber(e){
                ...
                if(_this.agree1 && _this.agree2){
                  ...
                  if(!_this.address.name && !_this.address2.name){
                    wx.showToast({ title: '请选择联系方式', icon: 'none', mask: true })
                    return;
                  }
                  if(!_this.date && !_this.movedate){
                    wx.showToast({ title: '请选择日期', icon: 'none', mask: true })
                    return;
                  }
                  if(!_this.timetext && !_this.movetimetext){
                    wx.showToast({ title: '请选择时间段', icon: 'none', mask: true })
                    return;
                  }
                  this.desc = this.desc.replace(/[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF][\u200D|\uFE0F]|[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF]|[0-9|*|#]\uFE0F\u20E3|[0-9|#]\u20E3|[\u203C-\u3299]\uFE0F\u200D|[\u203C-\u3299]\uFE0F|[\u2122-\u2B55]|\u303D|[\A9|\AE]\u3030|\uA9|\uAE|\u3030/ig, "");
                  let desc1 = `问题描述: 微信报单,移机, 备注:${_this.desc}`+'期望上门时间:'+_this.date+'  '+_this.timetext+'优先联系电话:'+_this.address.mobile+'微信授权手机号：'+ phone;

                  let desc2 = `问题描述: 微信报单,二次安装, 备注:${_this.desc}`+'期望上门时间:'+_this.movedate+'  '+_this.movetimetext+'优先联系电话:'+_this.address2.mobile+'微信授权手机号：'+ phone;

                  if(_this.movetimetext !='' && _this.movedate!='' && _this.address2.id && _this.timetext !='' && _this.date!='' && _this.address .id){
                    ...
                    saveOrder(datas).then(res=>{
                      const datas ={
                        addressId:_this.address2.id,
                        source:1,
                        orderDate:_this.movedate,
                        productId:_this.productId,
                        productSn:_this.productmodal,
                        timeRange:_this.movetimetext,
                        otherRequest:desc2,
                        type:6,
                        delivedFlag:1,
                        diagCode:39069,
                      ...
                      saveOrder(datas).then(res=>{
                        if (res.data.flag == 1100) {
                            const od={
                              ...
                              proName:_this.productType,
                              applCode:_this.productmodal,
                              expectationTime:_this.movedate,
                              expectationTimeType:_this.movetimetext,
                              userName:_this.address2.surname+_this.address2.name,
                              userMobile:_this.address2.mobile,
                              wehcatMobile:phone,
                              address:_this.address2.province+_this.address2.city+_this.address2.country+_this.address2.addr,
                              orderStatus:1
                            }
                            wxordersave(od).then(res=>{
                              wx.hideLoading();
                              ...
                              wx.removeStorageSync("address");
                              wx.removeStorageSync("selectPro");
                              reLaunch(`/pages/movemachinesuccess/main?address2=${JSON.stringify(_this.address2)}&address=${JSON.stringify(_this.address)}`)
                            })
                  ...
                  }else if((_this.timetext !='' && _this.date!='' && _this.address .id)){
                    const datas ={
                      addressId:_this.address.id,
                      source:1,
                      orderDate:_this.date,
                    ...
                    saveOrder(datas).then(res=>{
                      if (res.data.flag == 1100) {
                        ...
                        wx.removeStorageSync("address");
                        wx.removeStorageSync("selectPro");
                        reLaunch(`/pages/movemachinesuccess/main?address=${JSON.stringify(_this.address)}`)
                        })
                  ...
                  }else if((_this.movetimetext !='' && _this.movedate!='' && _this.address2.id )){
                    const datas ={
                      addressId:_this.address2.id,
                      source:1,
                      orderDate:_this.movedate,
                    ...
                    saveOrder(datas).then(res=>{
                      ...
                      if (res.data.flag == 1100) {
                        ...
                        wx.removeStorageSync("address");
                        wx.removeStorageSync("selectPro");
                        reLaunch(`/pages/movemachinesuccess/main?address2=${JSON.stringify(_this.address2)}`)
                        })
      ...
      movetimechange(e){
        this.movetimetext = this.movetimearray[e.mp.detail.value];
      },
      ...
      selectcontactor2(){
        navigate('/pages/contactWay/main?type=2');
      },
      textinput(e){
        this.num =e.mp.detail.value.length;
      },
      changePro(){
        navigate(`/pages/selectProduct/main?id=${this.pro.id}`)
      },
      faulttypechange(e){
