  ...
  <div class="repairnothousehold">
    ...
    <div class="contact2">
      <div class="panel contactpanel" @tap="servicestandar">
        <p>
          <span class="panel_name">客户服务收费标准</span>
          <span class="check_name">
            请查看
            <img src="../../../static/images/right_icon.png">
          </span>
...
<script>
import { navigate,reLaunch ,formatTime,arrayTime,formatNumber} from "../../../utils/index.js";
import {
...
export default {
  ...
  async onLoad(option) {
    ...
    let tiptime = wx.getStorageSync('doortime');
    if(now == tiptime){
      this.isclose = false;
    }
    ...
    await getInstallProductTypes(data).then(res => {
      for (let i = 0; i < res.data.data.length; i++) {
        if((res.data.data[i].typeName.indexOf("上下两门冰箱") != -1) || (res.data.data[i].typeName.indexOf("上下三门冰箱") != -1) || (res.data.data[i].typeName.indexOf("酒柜") != -1)){
  ...
  onShow() {
    ...
    if(this.productTypes.length !=0){
      ...
      if (pro) {
        ...
        if((this.pro.productName.indexOf("上下两门冰箱") != -1) || (this.pro.productName.indexOf("上下三门冰箱") != -1) || (this.pro.productName.indexOf("酒柜") != -1)) {
  ...
  methods: {
    ...
    close(){
      ...
      wx.setStorageSync('doortime', date)
    },
    preview(path){
      let list = [path];
      wx.previewImage({
    ...
    bind (id, fd, sn, type) {
      bindProduct({
        ...
        if (res.data.code == -1) {
          ...
          if((res.data.data.productName.indexOf("上下两门冰箱") == -1) && (res.data.data.productName.indexOf("上下三门冰箱") == -1) && (res.data.data.productName.indexOf("酒柜") == -1)){
              ...
              if (_this.agree1 && _this.agree2) {
                ...
                saveOrder(data).then(res => {
                  if (res.data.flag == 1100) {
                    ...
                    wx.removeStorageSync("address");
                    wx.removeStorageSync("selectPro");
                    reLaunch(`/pages/doorchangedirsuccess/main?tip=${JSON.stringify(_this.tip)}&address=${JSON.stringify(_this.address)}`)
                    })
    ...
    changePro() {
      wx.navigateTo({
        url: "/pages/selectProduct/main?type=5"
      });
    },
    timechange(e) {
