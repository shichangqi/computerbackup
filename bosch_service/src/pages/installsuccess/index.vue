<template>
    <div class="installsuccess">
      <div class="top">
        ...
        <div class="desc">
          <!--<p>您的预约安装订单已提交成功！您可在“服务记录”中查看相应状态~</p>-->
          <p>工程师最迟会在上门日期当天早上9：30前与您确认上门时间，请保持电话通畅。</p>
            <p>工程师上门信息将以短信发送至您手机，敬请留意。</p>
        </div>
        <!--<div class="remind">-->
          <!--<p>1. 短信会接收到工程师的信息请注意查收。</p>-->
          <!--<p>2.可在“服务记录”中查看最新服务状态。</p>-->
        <!--</div>-->
      </div>
      <div class="installtips" v-if="tip != '<p></p>'">
        <h5>安装提示</h5>
        <wxParse :content="tip"  />
      </div>
      <div class="userinfo">
        ...
        <div class="address"><label>安装地址：</label><p>{{address.province}}{{address.city}}{{address.country}}{{address.addr}}</p></div>
      </div>
...
<script>
  ...
  export default {
    ...
    onLoad(option){
      this.tip = JSON.parse(option.tip);
