...
waterrequest.interceptors.request.use((req) => {
  const token = wx.getStorageSync('token')
  debugger
  if (token) {
    req.headers['authorization'] = token;
...
waterrequest.interceptors.response.use(
  (response, promise) => {
    return promise.resolve(response)
  },
  (err, promise) => {
...
export  {request,bsrequest,notrequest,lightrequest,waterrequest}
